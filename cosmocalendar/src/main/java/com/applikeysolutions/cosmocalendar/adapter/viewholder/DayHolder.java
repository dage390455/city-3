...
public class DayHolder extends BaseDayHolder {
    ...
    private void select(Day day) {
        ...
        if (day.getDayOfWeek() == Calendar.SUNDAY) {
            state = SelectionState.SUNDAY;
        }
        if (day.getDayOfWeek() == Calendar.SATURDAY) {
            state = SelectionState.SATURDAY;
        }
    ...
    private void animateDay(SelectionState state, Day day) {
        if (day.getSelectionState() != state) {
            ...
            } else if (day.isSelectionCircleDrawed() && state == SelectionState.SATURDAY) {
                ctvDay.showAsStartCircle(calendarView, false);
            } else if (day.isSelectionCircleDrawed() && state == SelectionState.SUNDAY) {
                ctvDay.showAsEndCircle(calendarView, false);
            } else {
