...
public class ContractManagerActivityPresenter extends BasePresenter<IContractManagerActivityView> implements IOnCreate
    ...
    private final List<StatusCountModel> mSelectStatuslist = new ArrayList<>();
    private final List<StatusCountModel> mSelectTypelist = new ArrayList<>();
    ...
    private void initSelectData() {
        StatusCountModel im1 = new StatusCountModel();
        im1.statusTitle = "全部状态";
        im1.count = -1;
        im1.status = 0;
        mSelectStatuslist.add(im1);
        StatusCountModel im2 = new StatusCountModel();
        im2.statusTitle = "未签订";
        im2.count = -1;
        im2.status = 1;
        mSelectStatuslist.add(im2);
        StatusCountModel im3 = new StatusCountModel();
        im3.statusTitle = "已签订";
        im3.count = -1;
        im3.status = 2;
        mSelectStatuslist.add(im3);

        StatusCountModel im4 = new StatusCountModel();
        im4.statusTitle = "全部合同";
        im4.count = -1;
        im4.status = 0;
        mSelectTypelist.add(im4);
        StatusCountModel im5 = new StatusCountModel();
        im5.statusTitle = "企业合同";
        im5.count = -1;
        im5.status = 1;
        mSelectTypelist.add(im5);
        StatusCountModel im6 = new StatusCountModel();
...
//        getView().startAC(intent);
    ...
    public void doSelectTypeDevice(StatusCountModel item) {
        switch (item.status) {
            case 0:
                requestDataType = null;
                break;
    ...
    public void doSelectStatusDevice(StatusCountModel item) {
        switch (item.status) {
            case 0:
                requestDataConfirmed = null;
                break;
